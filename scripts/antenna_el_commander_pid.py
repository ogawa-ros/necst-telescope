...
class antenna_el_feedback:
    ...
    def init_ros(self):
        _publisher_conf = {
            "calculated_rate": ["/1p85m/el_speed", Float64, 1],
            "current_speed": ["/1p85m/el_current_speed", Float64, 1],
            "target_speed": ["/1p85m/el_target_speed", Float64, 1],
            "error": ["/1p85m/el_pid_hensa", Float64, 1],
        }
        _subscriber_conf = {
            "commanded_coord": ["/1p85m/el_cmd2", Float64, self.antenna_el_feedback, 1],
            "encoder_coord": ["/1p85m/el", Float64, self.antenna_el_encoder, 1],
        }
        self.Publisher = {
    ...
    def antenna_el_encoder(self, status: Float64):
        self.enc_coord = status.data
        self.deg_enc = self.enc_coord  # Alias.

    def antenna_el_pid(self, status: Float64):
